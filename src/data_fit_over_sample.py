...
class DataFit:
    def __init__(self, data_source='logistic_imputed',mode='normal'):
        ...
        # 创建结果保存目录
        self.results_dir = os.path.join(self.output_dir, 'modeling_oversample_results')
    ...
    def preprocess_data(self):
        ...
        X_train_oversample, y_train_oversample = smt.fit_resample(X_train_scaled, y_train_full)
        pdb.set_trace()

        # 转换为DataFrame
        self.X_train = pd.DataFrame(X_train_oversample, columns=selected_features)
        self.X_test = pd.DataFrame(X_test_scaled, columns=selected_features)
        self.y_train = y_train_oversample
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='2class', help='数据集模式')
