...
class ImprovedLogisticImputation:
    ...
    def impute_em_dataset(self, train_data, test_data):
        ...
        imputed_dir = self.output_dir / 'new_imputed'
        imputed_dir.mkdir(exist_ok=True)
        ...
        print("youxi")
        imputed_train.to_csv(imputed_dir / 'new_imputed_train.csv', index=False)

        
        imputed_test = self.mhandler.em_categorical_imputation(data = test_data)
        imputed_test.to_csv(imputed_dir / 'new_imputed_test.csv', index=False)
        print("yoo")
...
def main():
    ...
    # 加载训练数据
    train_path = handler.output_dir / 'final_encoded_train.csv'
    ...
    # 加载测试数据
    test_path = Path('Dataset/processed/test_processed/final_encoded_test.csv')
    test_data = None
    if test_path.exists():
